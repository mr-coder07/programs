...
bool isSparseMatrix(int matrix[][MAX_SIZE], int rows, int cols) {
...
void convertToTriplet(int matrix[][MAX_SIZE], int rows, int cols, vector<Triplet>& tripletList) {
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            if (matrix[i][j] != 0) {
                Triplet triplet = {i, j, matrix[i][j]};
